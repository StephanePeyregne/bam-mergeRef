...
bool parseHeader(string textHeader, string &headerHD, vector<string> &headerSQ, vector<string> &headerRG, vector<string> &headerPG, vector<string> &headerCO)
{
        string line;
...
int main( int argc, const char *argv[] ) 
{
	...
	//poptSetOtherOptionHelp( optCon, "[OPTIONS]* <inputfile1> <inputfile2> <outputfile> <trashfile>" ) ;
	poptSetOtherOptionHelp( optCon, "[OPTIONS]* -a <reference name 1> -b <reference name 2> <inputfile1> <inputfile2> <outputfile>" ) ;
	...
	// BamMultiReader *mReader = new BamMultiReader; // Create reader
	...
	if(!mFile1->Open(infile1))
	{
		...
		delete mFile2;
		delete mOutFile;
		if(mTrashFile != nullptr) delete mTrashFile;
		return 1;
	}
	...
	string textHeader1 = mFile1->GetHeader().ToString();
        ...
        int i = 0, j = 0;
        regex ANregex("\tAN:[0-9A-Za-z][0-9A-Za-z\\*\\+\\.@_\\|-]*(,[0-9A-Za-z][0-9A-Za-z\\*\\+\\.@_\\|-]*)*");
        smatch matchAN;

        regex SNregex("\tSN:([0-9A-Za-z][0-9A-Za-z\\*\\+\\.@_\\|-]*)");
        smatch matchSN;

        //const char* ref1Name = "ref1";//REMOVE THIS
        //const char* ref2Name = "ref2";//REMOVE THIS
...
//              cout << headerSQ1[i] << headerSQ2[j] << "\n";
                if(strverscmp(headerSQ1[i].c_str(),headerSQ2[j].c_str())==0)
                {
                        ...
                        {
                                cerr << "Error: A header line (@SQ) is missing its SN tag in both input files." << endl;
                                mFile1->Close();
                                mFile2->Close();
                                delete mFile1;
                                delete mFile2;
...
//              cout << headerSQ2[j] << "\n";
        ...
        //use unique for removing consecutive identical elements //see how to do here: http://www.cplusplus.com/reference/algorithm/unique/
        ...
        //headerPGout.insert( headerPGout.end(), headerPG1.begin(), headerPG1.end() ); //OLDEST @PG SHOULD BE THE LAST AND NEWEST @PG SHOULD BE THE FIRST

        //CONCATENATE ALL THE VECTOR IN mHeaderOut
        textHeaderOut += headerHDout;
        textHeaderOut += "\n";
        for(auto str : headerSQout) textHeaderOut += str + "\n";
        for(auto str : headerRGout) textHeaderOut += str + "\n";
        textHeaderOut += newPG + "\n";
        for(auto str : headerPG1) textHeaderOut += str + "\n";
        //for(int i = 0; i < headerPG1.size(); i++)
        //{
        //      cout << headerPG1[i] << "\n";
        //}
        for(auto str : headerPG2) textHeaderOut += str + "\n";
        for(auto str : headerCO1) textHeaderOut += str + "\n";
        for(auto str : headerCO2) textHeaderOut += str + "\n";


	// Open output file
	//const SamHeader header = reader.GetHeader();
	//const RefVector references = reader.GetReferenceData();
	if(!mOutFile->Open(outfile, SamHeader(textHeaderOut), mFile1->GetReferenceData())) //CHANGE WHICH HEADER IS WRITTEN TO mHeaderOut
	{
	...
	//mOutFile->SetCompressionMode(BamWriter::Uncompressed );

	if(mTrashFile != nullptr)
	{
		if(!mTrashFile->Open(trashFileName, SamHeader(textHeaderOut), mFile1->GetReferenceData())) //MAKE THIS OPTIONAL ! + ADD A SUFFIX
		{
		...
		//free(trashFileName); //free the memory from malloc
		//trashFileName=nullptr;
...
//		{
			//cerr << "Error: The entries of both BAM files are not identical. Either you do not have the same sequences in both files or you did not sort the entries by names." << endl;
			//poptPrintUsage( optCon, stderr, 0 ) ;
			//error = 1;
			//break;
//		cout << aln1.Name << "\t" << aln2.Name << "\n";
		if(aln1.Name!=aln2.Name)
		{
			...
			if((readLine1) // If lines in File 1
				&& ((strverscmp(aln1.Name.c_str(),aln2.Name.c_str())<0) || readLine2 == false )) // And name1 < name2 or file 2 empty
			{
				//cout << "Fichier 1\n";
				aln = &aln1;
			...
			if(strverscmp(aln->Name.c_str(),previousAlnName.c_str())<=0) // from string.h // If not sorted // verify that it is the right order (maybe change to <0)
			{
			...
			if(aln->IsPaired())
			{
				...
				{
					cerr << "Error: A widow was encountered in file " << currentFileNumber << ". Check that all paired reads have a mate or sort your BAM files by names" << endl;
					poptPrintUsage( optCon, stderr, 0 );
					error = 1;
					break;
				}
		...
		else // both files are treated simultaneously
		{
			...
			if(strverscmp(aln1.Name.c_str(),previousAlnName.c_str())<=0)//(aln1.Name<=previousAlnName) //(strverscmp(aln1.Name.c_str(),previousAlnName.c_str())<=0) from string.h // If not sorted // verify that it is the right order (maybe change to <0)
			{
				// Data are not sorted get out
				// Or maybe pair that was not previously detected with IsPaired() ???
				cerr << "Error: Please sort the entries of your BAM files by names. 3" << endl;
			...
			if(aln1.IsPaired()&&aln2.IsPaired())
			{
				...
				BamAlignment aln3;
				BamAlignment aln4;
				if(	!mFile1->GetNextAlignment(aln3)
					|| !mFile2->GetNextAlignment(aln4))
				{
					cerr << "Error: Reached the end of the file (or could not read the next entry) without finding a mate. Check that all paired reads have a mate" << endl;//SHOULD BE ABLE TO DIFFERENTIATE BETWEEN END OF FILE AND A WRONG LINE...
					poptPrintUsage( optCon, stderr, 0 ) ;
					error = 1;
					break;
				}
				...
				{
					cout << aln1.Name << "\t" << aln3.Name << "\t" << aln2.Name << "\t" << aln4.Name << "\n";
					cerr << "Error: A widow was encountered. Check that all paired reads have a mate or sort your BAM files by names" << endl;
					poptPrintUsage( optCon, stderr, 0 ) ;
					error = 1;
					break;
				}*/
				
				// Update previousAlnName is not required because aln3/aln1 and aln4/aln2 have the same name
				// Algorithm here
...
//							continue; //IS THAT CORRECT TO REMOVE THIS LINE?
						}
						else // aln2 is mapped
						{
							alnKeep = &aln2;
					...
					else //aln1 is mapped
					{
						...
						{
							if(aln1.Position!=aln2.Position || !isSameCigar(aln1.CigarData,aln2.CigarData))
							{
								if(mTrashFile != nullptr)
								{
									//cout << aln1.Name << "\t" << aln1.Position << "\t" << aln2.Name << "\t" << aln2.Position << "\n";
									aln1.AddTag("RN","i",1);
							...
							alnKeep->AddTag("RN","i",12);//add tag that both aln1 and aln2 were mapped
						}
					}
						...
						else
						{
								...
								|| (!aln1.IsFirstMate() && !aln2.IsFirstMate()))
							{
								if(aln1.Position!=aln2.Position 
									...
									|| !isSameCigar(aln3.CigarData,aln4.CigarData))//I AM ASSUMING THAT THIS WORKS EVEN WHEN THE READ IS UNMAPPED, CHECK THAT!!
								{
									if(mTrashFile != nullptr)
									{
										aln1.AddTag("RN","i",1);
